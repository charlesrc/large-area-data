...
% Unpatterned.
load('processed_data/IL/high-magnification/90-kVp/razor_blade_sample2_90kVp_12W_3.0s_hi-mag_unpatterned_results.mat');
...
text(-1.8, 1.7, ['Bare, \sigma = ' num2str(2*sqrt(2*log(2))*x(3), '%.3f') ' \pm ' num2str(2*sqrt(2*log(2))*se(3), '%.3f') ' mm'], 'Color', 'r', 'FontSize', 12);

% Patterned.
load('processed_data/IL/high-magnification/90-kVp/razor_blade_sample2_90kVp_12W_3.0s_hi-mag_patterned_results.mat');
ROI = rescaleESF(ROI, x);
shadedErrorBar(shiftY(y_position, x), mean(ROI, 1, 'omitnan'), std(ROI, 1, 'omitnan'), ...
    'lineProps', {'b-'});
plot(shiftY(y_position, x), rescaleESF(ESF(x, y_position), x), 'b--', 'LineWidth', 1.5);
text(-1.8, 1.4, ['PhC, \sigma = ' num2str(2*sqrt(2*log(2))*x(3), '%.3f') ' \pm ' num2str(2*sqrt(2*log(2))*se(3), '%.3f') ' mm'], 'Color', 'b', 'FontSize', 12);
...
box on;
% exportgraphics(f, 'Figure_S13_90kVp_sample2_plot.pdf');
